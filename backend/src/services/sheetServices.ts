 ...
 */
export const getGSheetByIdService = async (
  sheetId: string,
  oauth2Client: any
): Promise<any> => {
  try {
    ...
    if (error instanceof Error) {
      throw new Error(
        `Failed to retrieve Google Sheet with ID ${sheetId}: ${error.message}`
      );
    } else {
      throw new Error(
        `Failed to retrieve Google Sheet with ID ${sheetId}: ${error}`
      );
    }
  }
};
 ...
 */
export const getSheetMetadataService = async (
  spreadsheetId: string,
  auth: any
): Promise<any> => {
  const sheets = google.sheets({ version: "v4", auth });
