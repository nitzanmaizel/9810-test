...
export async function fetchAPI<T>(endpoint: string, options: FetchAPIOptions = {}): Promise<T> {
  const { method = 'GET', headers = {}, body = null, queryParams = {}, signal, responseType = 'json'} = options;
  ...
  try {
    ...
    const fetchOptions: RequestInit = { method, headers: fetchHeaders, signal, credentials: 'include' };
